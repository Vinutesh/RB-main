...
export function HomePage() {
  ...
  return (
    <>
      ...
      <section className="relative min-h-[100svh]">
        ...
        <div className="relative mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-center px-4 pb-40 pt-20 text-center sm:px-6 lg:px-8">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
            <Link
              to="/contact"
              className="inline-flex min-h-12 items-center rounded-full border border-white px-7 text-sm font-medium text-white transition hover:bg-white hover:text-black"
            >
              Get in touch
            </Link>
            <Link
      ...
      <section className="border-y border-white/10 bg-black/35 py-6">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:px-8">
          ...
          <div className="overflow-hidden whitespace-nowrap">
            <div className="marquee-track inline-flex min-w-full gap-10 text-sm text-[var(--text-soft)]">
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {portfolioPreview.map((image, index) => (
            <div key={image} className="mx-auto h-48 w-48 overflow-hidden rounded-full border border-white/15 transition duration-300 hover:scale-105">
              <img src={image} alt={`Portfolio preview ${index + 1}`} loading="lazy" className="h-full w-full object-cover" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          {services.map((service) => (
            <article key={service.title} className="overflow-hidden rounded-3xl border border-white/10 bg-[#161616]">
              ...
              <div className="p-6">
                ...
                <Link
                  to={service.cta}
                  className="mt-5 inline-flex min-h-11 items-center rounded-full border border-white px-5 text-sm text-white transition hover:bg-white hover:text-black"
                >
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {projectShowcase.map((project) => (
            <article key={project.name} className="overflow-hidden rounded-3xl border border-white/10 bg-[#141414]">
              <img src={project.image} alt={project.name} className="h-60 w-full object-cover" loading="lazy" />
              <div className="p-6">
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4 pb-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <a
            ...
            className="inline-flex min-h-12 items-center rounded-full border border-white px-5 py-3 text-sm"
          >
